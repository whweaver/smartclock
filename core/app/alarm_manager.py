...
class AlarmManager:
    REFRESH_PERIOD = datetime.timedelta(minutes=1)
    ...
    def process(self):
        ...
        if cur_time - self.last_refresh >= AlarmManager.REFRESH_PERIOD:
            self.refresh()
...
class Alarm:
    def __init__(self, db_alarm, manager):
        ...
        self.alarm_time = self.get_next_alarm_time(ignore_days=True)
        self.sunrise = AlarmSunrise(self.alarm_time, db_alarm.sunrise_fade, db_alarm.sunrise_brightness, db_alarm.sunrise_color)
        self.music = AlarmMusic(self.alarm_time, db_alarm.music_fade, db_alarm.music_account, db_alarm.music_playlist, db_alarm.music_vol)
    ...
    def get_next_alarm_date(self, ignore_days=False):
        ...
        if len(self.db_alarm.days) > 0 or ignore_days:
            cur_time = datetime.datetime.now()
            if self.db_alarm.time >= cur_time.time().replace(second=0, microsecond=0):
    ...
    def refresh(self, db_alarm):
        self.db_alarm = db_alarm
        if self.alarm_time.time() != db_alarm.time:
            self.alarm_time = self.get_next_alarm_time(ignore_days=True)
            self.sunrise.reset(self.alarm_time)
            self.music.reset(self.alarm_time)
        if self.sunrise is not None:
            self.sunrise.refresh(self.alarm_time, db_alarm.sunrise_fade, db_alarm.sunrise_brightness, db_alarm.sunrise_color)
        if self.music is not None:
            self.music.refresh(self.alarm_time, db_alarm.music_fade, db_alarm.music_account, db_alarm.music_playlist, db_alarm.music_vol)
    ...
    def reset(self):
        self.alarm_time = self.get_next_alarm_time()
        self.sunrise.reset(self.alarm_time)
        self.music.reset(self.alarm_time)
...
class AlarmComponent:
    def __init__(self, alarm_time, duration):
        ...
        self.alarming = False
        self.nominal_fade_duration = duration
    ...
    def refresh(self, alarm_time, duration):
        self.nominal_fade_duration = duration
        self._set_times(alarm_time, duration)

    def snooze(self, snooze_time):
        if self.alarming:
            if snooze_time > self.nominal_fade_duration:
                duration = self.nominal_fade_duration
            else:
    ...
    def reset(self, new_alarm_time):
        ...
        if new_alarm_time is not None:
            self._set_times(new_alarm_time, self.nominal_fade_duration)
...
class AlarmSunrise(AlarmComponent):
    ...
    def refresh(self, alarm_time, duration, brightness, color):
        self.brightness = brightness
        self.color = color
        super().refresh(alarm_time, duration)
...
class AlarmMusic(AlarmComponent):
    ...
    def refresh(self, alarm_time, duration, account, playlist, volume):
        self.volume = volume
        if self.account != account or self.playlist != playlist:
            self._clear_effects()
            self.account = account
            self.playlist = playlist
        super().refresh(alarm_time, duration)
